...
function convertToArray1(routeSelected) {
  const routeData = [
    { route_id: "routeId", route_cities: "CityA,CityB,CityC" },
  ];
...
let selectedSeats = []; // Store selected seat IDs
...
function selectSeat(evt) {
  if (
    ...
    // Update seatInput value with selected seats separated by comma
    seatInput.value = selectedSeats.join(",");

    // Calculate amount only if at least one seat is selected
    if (selectedSeats.length > 0) {
      ...
      const amount =
        (citiesArr.indexOf(destSelected) - citiesArr.indexOf(sourceSelected)) *
        parseInt(stepCost) *
        selectedSeats.length;
      document.querySelector("#bookAmount").value = amount;
    } else {
      // If no seats are selected, clear the amount
